...
| Băng đạn / dự trữ / reload tự động; shotgun nạp từng viên 1.05s/viên | **Có** |
...
| 66 | **Nhả tay khỏi giữ-bắn không đạt được gì cả — và còn bị phạt NẶNG HƠN là không nhả.** Nhả tay xong súng **vẫn ra ngoài** (`gunHoldSec` 0.30s mới cất), nên cú chạm tiếp theo bị `_holdTimer` ném thẳng lại vào `HOLD_FIRE` sau 130ms và bắn lại **ngay lập tức** vì không có độ trễ rút súng. Đo: nhả tay rồi quẹt tốn **72px** mới ra dao, còn **không** nhấc tay mà vẩy thẳng chỉ tốn **48px**. Thao tác dứt khoát hơn thì bị tính giá cao hơn | Thêm **cửa sổ đổi vũ khí** `weaponSwitchWindow` 300ms sau khi nhả tay: quãng đường chốt rút về `slideMinLength`, và súng phải đợi `reholdDelay` (220ms) mới được tóm lại ngón tay |
| 67 | **LUẬT KHOÁ ĐÃ HẾT LÝ DO TỒN TẠI — nó sinh ra để xử lý một sự mơ hồ không còn nữa.** Luật khoá có khi hold còn là **hold-and-drag**: giữ để bắn *vào chỗ ngón tay chỉ*, nên di chuyển ngón vừa có thể là rê tâm ngắm vừa có thể là chém. Khi hold thành **auto-aim** (lỗi #58) thì ngón tay không còn việc gì để làm trong lúc giữ — mọi chuyển động chỉ còn **một** cách hiểu, mà luật khoá vẫn ở đó thu thuế. Ngưỡng phá khoá `1.15 × slideVel` = **414 px/s** còn *cao hơn* ngưỡng nhận quẹt thường: đo được cú kéo dứt khoát ở **150–200 px/s bị nuốt hoàn toàn** — vẫn bắn, không ra dao | **Bỏ luật khoá**, thay bằng cặp đối xứng trong cùng một cú chạm: **ĐI** quá `holdBreakTravel` (26px) tính từ điểm dừng gần nhất → ra dao ngay, **không cần vận tốc**; **DỪNG** quá `sliceStillMs` (150ms) → súng tự rút ra bắn. Không còn lần nào phải nhấc tay để đổi vũ khí. Chống chém oan chuyển từ *ngưỡng vận tốc cao* sang **mốc tự đặt lại** (`holdRestVel` 90 px/s): xê dịch chậm không cộng dồn — đo giữ bắn rồi trượt 40px trong 1.2s (39 px/s) → **0 nhát chém oan**. Đo chuỗi một cú chạm không nhấc tay lần nào: giữ bắn (đạn 99→96) → vẩy (chém) → dừng (96→93) → vẩy (chém) → dừng (93→90) |
| 68 | **"Dừng tay" mà vẫn tụt stamina, và tụt VĨNH VIỄN.** Stamina bị trừ theo *trạng thái* `slicing` chứ không theo việc có chém hay không — 34/giây — và mỗi frame nó đặt `staminaIdle` về 0, nên còn ở trạng thái chém là **không bao giờ hồi lại được**. Bình thường không lộ vì `SLIDE_CONT` chỉ kết thúc lúc nhấc tay. Nhưng luật mới (#67) tạo ra một trạng thái mới: ngón tay dừng mà súng **không rút ra được** (băng rỗng đang nạp) → quay về `SLIDE_CONT` và ngồi đó tụt stamina trong khi không chém gì cả | Tách **"đang ở chế độ chém"** khỏi **"ngón tay đang đi"**: `_endSlicing()` tắt drain NGAY khi ngón dừng, không đợi xem súng có ra được không; ngón đi tiếp thì bật lại. Đo ca khó (băng rỗng + đang nạp): chém 8 nhát (stam 100→84) → dừng tay → drain tắt, **stamina hồi 80→92** → đi tiếp → chém thêm 12 nhát |
| 69 | **Nhát chém đầu tiên của một cú quẹt chỉ hiện ra khi ngón tay đã đi 72px — nhìn như nó không bắt đầu ở chỗ mình chạm.** Vệt chém *có* vẽ từ đúng điểm chạm (`_blade` đẩy `s.from → s.to` vào trail), nhưng trước lúc chốt thì **không có gì hiện ra cả**, nên vệt xuất hiện lúc ngón đã ở xa — đọc ra như nhát chém trễ hơn ngón tay. Lộ rõ nhất ở cú quẹt **thứ hai liên tiếp**: quẹt 1 chuyển sang chém liên tục với mỗi 14px một nhát, rồi quẹt 2 lại bắt trả 68px từ đầu. Thêm nữa `weaponSwitchWindow` (#66) chỉ tính từ lúc kết thúc **giữ-bắn**, nên quẹt→quẹt không được giảm giá gì | Hai sửa. **(a)** Chốt sớm khi **ngón tay còn đang bay**: `slideCommitLength` 68px phải đặt cao vì nó chặn tap trượt tay (đo được tới 55px), nhưng thứ tách được hai cái đó không phải quãng đường mà là **đã dừng hay còn bay** — đúng luật đã dùng ở `_up`. Ở 45px thì tap trượt tay đã giảm tốc gần hết (đo: 839 → 570 → 291 → 90 px/s) còn cú quẹt thật vẫn full tốc, nên chỉ cần hỏi "còn bay không". **(b)** Cửa sổ đổi vũ khí tính từ lúc kết thúc **bất kỳ động tác chiến đấu nào**, không riêng giữ-bắn — nhưng **KHÔNG tính cú tap**, vì hai tap liên tiếp mà hạ ngưỡng bảo vệ của chính cú tap thứ hai thì rơi lại đúng lỗi #56. Đo: nhát đầu ra ở **48px** thay vì 72px, cả cú quẹt nguội lẫn cú quẹt thứ hai liên tiếp; tap trôi tay 0 / 30 / 55px vẫn **0 nhát chém oan**, súng vẫn nổ cả ba |
| 70 | **Xong phòng trong lúc đang giữ bắn thì súng bắn hết sạch đạn vào khoảng không.** Màn hình Cổng là một khoảnh khắc KHÔNG chiến đấu, nhưng `step()` vẫn chạy và `holdPt` vẫn còn nguyên — súng cứ thế nhả đạn suốt lúc người chơi ngồi đọc thẻ nâng cấp. Đo: giữ bắn rồi dọn sạch phòng → băng 22 viên **cạn sạch sau 3.7 giây**, `holdPt` vẫn `true`. Cùng lỗi này còn một mặt nữa: đang chém mà xong phòng thì `slicing` vẫn bật, stamina tụt về 0. Và **không thể trông chờ `pointerup` dọn dẹp hộ**: ngón tay nhấc lên TRÊN LỚP UI chứ không phải trên canvas, nên router không bao giờ nhận được | `stopCombatInput()` gọi ngay lúc `_roomCleared()`: xoá `holdPt`, tắt `slicing`, cất súng, và `input.reset()` để router **quên** cú chạm đang theo dõi. Đo lại: **0 viên phí**, router về `IDLE`, và băng được nạp đầy trước khi vào phòng sau |
| 71 | **Thẻ Băng Đạn làm shotgun nạp mỗi viên NHANH HƠN.** Nhịp nạp từng viên tính bằng `reloadTime / magMax` — mà `magMax` thì nâng cấp được. Đo: băng 5 → 0.75s/viên, băng 10 → **0.38s**, băng 20 → **0.19s**. Băng to gấp bốn thì mỗi viên vào nhanh gấp bốn, đúng ngược với thực tế và đúng kiểu lỗi **neo vào một con số nâng cấp được** đã gặp ở Cướp Đạn (#55) | Nhịp mỗi viên thành **hằng số của khẩu súng**: `archetypeSpec.shotgun.shellReloadSec` = **1.05s** (từ 0.75s — người chơi báo nạp quá nhanh). Băng to hơn = nạp lâu hơn vì có nhiều viên hơn phải vào, còn nhịp thì không đổi cả run. Đo: 1.05s cho cả magMax 5 / 10 / 20 |
| 72 | **BỎ Nạp Hoàn Hảo.** Nó đòi đúng một thứ mà game này không cho phép: **nhìn xuống thanh nạp**. Lỗi #40 đã phải dời thanh nạp lên sát tâm ngắm vì hết đạn là lúc căng nhất — nhưng dời chỗ chỉ chữa triệu chứng, bản thân cơ chế vẫn là *canh một cửa sổ 0.25s bằng mắt, đúng lúc mắt đang bận nhất*. Một cơ chế mà cách chơi tối ưu là phớt lờ nó đi thì không phải chiều sâu, là thuế | Gỡ toàn bộ: `tryPerfectReload` / `reloadWin` / `perfectMag` / `reloadTapped` / `reloadStats`, vạch xanh trên vòng nạp (`#rlRing .win`), dòng thống kê cuối run, nút gọi trong `main.js`. Hết băng là **tự nạp**, không phạt, không thưởng. Quyết định được giữ lại là cái duy nhất không cần nhìn xuống: **nạp nốt cho đầy hay cắt ngang bắn ngay** — nó nhìn vào đám quái để quyết. Talent Cò Vàng và relic Găng Thợ Rèn đổi sang buff viên đầu mỗi băng |
| 73 | **Vạch đích không có hình hài gì trong thế giới.** Chạy hết 150m là người chơi đứng sựng lại **giữa một đám quái còn sống** (đo: 29 con còn sống, **20 con ngay phía trước mặt**) rồi một lớp UI phủ lên. Ngã Ba Hầm — thứ `08` mục 3 gọi là "quyết định KHÔNG GIAN" — thực ra không tồn tại trong không gian: nó là hai cái nút HTML. Người chơi không *tới* đâu cả, họ bị một cái menu chặn lại giữa lúc đang bắn | Cuối hành lang thành một **bức tường có hai cửa**, hiện ngay lúc chạm 150m ở `doorApproachM` 22m phía trước; quái còn sống **phía trước giải tán** (không rơi vàng — giết trước vạch đích mới ăn, nên 150m là hạn chót thật), người chơi **chạy nốt** 12m rồi dừng ở `doorStopGapM` 10m trước cửa và Cổng mới mở. Đo: 20 con phía trước → **0**, dừng đúng 10m, chọn cửa → phòng 1→2, cửa biến mất, chạy tiếp bình thường |
| 74 | **Hình học cửa sai và khoảng đứng sai — hai lỗi che nhau.** Bề rộng trụ giữa tính bằng `HW - 2*beRong - 2*DW` với `beRong = (HW-2*DW)/2` → luôn ra **0**: hai cửa dính thành một lỗ hổng 4m. Và dừng ở 3.2m thì khung nhìn **ngang** chỉ thấy 2.1m (fov 72° là góc DỌC, màn dọc nên ngang hẹp hơn ~2 lần) — kể cả hình học đúng thì cũng không thấy nổi một cánh cửa | Tính thẳng từ vị trí cửa: `beRong = HW/2 - DX - DW/2`, `truGiua = 2*(DX - DW/2)`. Dừng ở **10m** và kê hai cửa sát nhau hơn (`DX` 1.8m) để **cả hai cùng nằm trong một khung nhìn** — không thì "chọn giữa hai cửa" lại thành hai cái nút, chỉ khác là bằng polygon |
| 75 | **Nỏ bắn ít mục tiêu hơn shotgun MÀ còn ít đạn hơn — nó bị trội hoàn toàn.** Dự trữ tính bằng `mag × 1.5`, mà nỏ băng **1 viên**, nên nó rơi xuống sàn `reserveMinShots` = 6 — một con số bắt ra từ không khí. Đo **số mạng cả kho đạn mua được**: nỏ **14**, shotgun 64, rifle 63, súng lục 25. Chọn nỏ là chọn một ván ngắn hơn 4.5 lần, không phải chọn một lối chơi khác. Gốc vấn đề: **"1.5 băng phụ" vô nghĩa khi khẩu súng nạp lại sau mỗi phát** — "băng" không còn là đơn vị của cái gì cả, nên mọi con số neo vào nó đều là số ngẫu nhiên | Với khẩu băng < `magSmallThreshold` (4), neo dự trữ vào **SỐ MẠNG** (`reserveKillsFloor` = 42) — thứ duy nhất có nghĩa giống nhau ở mọi khẩu: `số phát = ceil(42 / killsPerShot)`, dùng luôn `killsPerShot` mà `normalize_balance.ps1` đã tính sẵn (có cả `pellets`, `pierce`, `killEff`). Nỏ: dự trữ **6 → 21**, số mạng **14 → 43**. Vẫn dưới rifle/shotgun — đúng, vì nó không phải khẩu dọn vùng — nhưng 22 phát × 1.18s = **25.9 giây bắn trong phòng 35.7 giây**, tức bắn liên tục thì vẫn cạn ở một phần ba cuối: P2 còn nguyên hiệu lực. Ngưỡng `magSmallThreshold` giờ đọc từ data ở **cả** `normalize_balance.ps1` lẫn cổng audit, thay vì hai chỗ cùng hardcode số 4 |
